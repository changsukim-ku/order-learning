...
if __name__ == '__main__':
    ...
    parser.add_argument('--chain', type=float, default=0)
    parser.add_argument('--delta', type=float, default=0)
    parser.add_argument('--dataset', type=str, default='dataset', help='path to saving model')
    parser.add_argument('--ckpt_dir', type=str, default='ckpt', help='path to saving model')
    parser.add_argument('--test_model_path', type=str, default='test', help='VGG_comp_all when fine-tuning')
    parser.add_argument('--ref_path', type=str, default='ref', help='VGG_comp_all when fine-tuning')
    parser.add_argument('--data_path', type=str, default='E:\Age_dataset')
    ...
    experimental_setting = 'Supervised'
    args.chain = 1
    ...
    if args.dataset == 'Balanced':
        ...
        # Comparator Model
        args.ckpt_dir = 'models/Balanced/%s' % experimental_setting
        ...
        print('Experimental Setting : ', experimental_setting)
    ...
    if args.dataset == 'Morph':
        ...
        for fold_num in range(5):
            ...
            args.fold = fold_num
            # Comparator Model
            args.ckpt_dir = 'models/MORPH_Setting%s' % (experimental_setting)
            args.test_model_path = 'Fold%d.ckpt' % (args.fold)
            # Reference Data
            args.ref_path = 'Fold%d' % (args.fold)
            ref_data = pd.read_csv(os.path.join(args.ckpt_dir, args.ref_path + '_ref_data.txt'), sep=' ')
            # Train & Test Data
            if experimental_setting == 'A':
                train_data = pd.read_csv(os.path.join('index', 'MORPH_Setting%s' % (experimental_setting),
                                                      'Setting%s_fold%d' % (
                                                          experimental_setting, args.fold) + '_train.txt'))
                val_data = pd.read_csv(os.path.join('index', 'MORPH_Setting%s' % (experimental_setting),
                                                    'Setting%s_fold%d' % (
                                                    experimental_setting, args.fold) + '_test.txt'))

            elif experimental_setting == 'B':
                fold_list = [0, 1, 2]
                fold_list.remove(args.fold)

                train_data = pd.read_csv(os.path.join('index', 'MORPH_Setting%s' % (experimental_setting),
                                                      'Setting%s_fold%d' % (experimental_setting, args.fold) + '.txt'),
                                         sep=" ")
                val_data_1 = pd.read_csv(os.path.join('index', 'MORPH_Setting%s' % (experimental_setting),
                                                      'Setting%s_fold%d' % (
                                                      experimental_setting, fold_list[0]) + '.txt'),
                                         sep=" ")
                val_data_2 = pd.read_csv(os.path.join('index', 'MORPH_Setting%s' % (experimental_setting),
                                                      'Setting%s_fold%d' % (
                                                      experimental_setting, fold_list[1]) + '.txt'),
                                         sep=" ")
                val_data = pd.concat([val_data_1, val_data_2]).reset_index(drop=True)

            else:
                data = pd.read_csv(os.path.join('index', 'MORPH_Setting%s' % (experimental_setting),
                                                'Setting%s' % (experimental_setting) + '.txt'), sep="\t")
